const Neo4jD3 = (_selector, _options) => {
    ...
    const zoom = d3.zoom()
        .scaleExtent(options.scaleExtent || [0.3, 5])
        .on('zoom', function () {
    ...
    function appendNodeToGraph() {
        ...
        if (options.icons || (options.name && options.name.show)) {
            appendTextToNode(n);
        }
    ...
    function appendOutlineToNode(node) {
        return node.append('circle')
            ...
            .style('fill', function (d) {
                let color = options.nodeOutlineFillColor ? options.nodeOutlineFillColor : class2color(d.labels[0]);
                d.color = color
                return color
            })
    ...
    function appendTextToNode(node) {
        return node.append('text')
            ...
            .html(function (d) {
                var _icon = icon(d);
                return _icon ? '&#x' + _icon : (d.name || d.id);
            });
    }
    ...
    function appendRelationship() {
        return relationship.enter()
            ...
            .on('mouseenter', function (d) {
                ...
                if (typeof options.onLinkMouseEnter === 'function') {
                    options.onLinkMouseEnter(d);
                }
            })
            .on('mouseleave', function (d) {
                if (info) {
                ...
                if (typeof options.onLinkMouseLeave === 'function') {
                    options.onLinkMouseLeave(d);
                }
            })
    }
    ...
    function initSimulation() {
        // let width = document.querySelector(_selector).clientWidth 
        // let height = document.querySelector(_selector).clientHeight;
        var simulation = d3.forceSimulation()
            ...
            .force('link', d3.forceLink().id(function (d) {
                return d.id;
            }).distance(options.distance || 10).strength(options.strength || 0.5))
            .force('center', d3.forceCenter(
                svg.node().parentElement.parentElement.clientWidth / 2, svg.node().parentElement.parentElement.clientHeight / 2
            ))
            .on('tick', function () {
                tick();
            })
    ...
    function neo4jDataToD3Data(data) {
        ...
        // let obj = {}
        // graph.nodes = graph.nodes.reduce((cur, next) => {
    ...
    */
    ...
    function updateInfo(d) {
        clearInfo();
        if (options.formatter) formatter(d)
        if (d.labels) {
    ...
    function zoomFit(transitionDuration) {
        ...
        let svgScale = 0.85 / Math.max(width / fullWidth, height / fullHeight);
        let svgTranslate = [fullWidth / 2 - svgScale * midX, fullHeight / 2 - svgScale * midY];
